...
inline bool pressEdge(uint32_t now, bool pressedNow) {
  if (pressedNow && !lastPressed && (now - lastEdgeMs > DEBOUNCE_MS)) {
    lastEdgeMs = now;
    lastPressed = pressedNow;
    return true;
  }
  lastPressed = pressedNow ? lastPressed : lastPressed; // keep until releaseEdge updates
  return false;
}

inline bool releaseEdge(uint32_t now, bool pressedNow) {
  if (!pressedNow && lastPressed && (now - lastEdgeMs > DEBOUNCE_MS)) {
    lastEdgeMs = now;
    lastPressed = pressedNow;
    return true;
...
void loop() {
  ...
  switch (state) {
    ...
    case WAIT_FOR_PRESS: {
      ...
      // keep lastPressed updated for edges
      if (pressedNow && !lastPressed) { lastPressed = true; }
    } break;

    case PRESSED_HELD: {
      // Keep it green while the user is pressing
  ...
  // Update lastPressed when not handled by edge helpers
  if (!pressedNow && lastPressed && (now - lastEdgeMs > DEBOUNCE_MS)) {
    lastPressed = false;
  }
}
